...
def save_language_level(username, level, db, cur):
    try:
        ...
        if level == "Intermediate":
            start_quiz_page_spanish_inter(username)
        elif level == "Advanced":
            start_quiz_page_spanish_adv(username)
        elif level == "Beginner":
            ssession_espbeginner(username)
...
def start_quiz_page_spanish_inter(username):
    os.system("python quiz_intermediate.py")

def start_quiz_page_spanish_adv(username):
    os.system("python quiz_advanced.py")

def ssession_espbeginner(username):
    os.system("python session_esp_beg.py")

def show_language_level_selection(username):
    level_selection_window.title("Select Your Spanish Proficiency Level")
