...
def display_question(window, question_data, question_index, total_questions, language_id, session_number, db, cur, img_bg, results):
    ...
    play_question_button = Button(window, image=play_img, command=lambda: play_tts(question_text, 'fr'), bg="white", borderwidth=0)
    play_question_button.place(x=1251, y=109)

    selected_option = IntVar()  # Use IntVar to store the selected option
    buttons = []
    ...
    for i, (option, x_pos, y_pos) in enumerate(options, start=1):
        option_button = Button(window, text=option, bg="white", fg="black", cursor="hand2", font=("Helvetica", 35), width=15, height=2, borderwidth=3, wraplength=400, command=lambda opt=i: select_option(opt, selected_option, buttons))
        option_button.place(x=x_pos, y=y_pos)
        buttons.append(option_button)
        play_option_button = Button(window, image=play_img, bg="white", borderwidth=0, command=lambda opt=option: play_tts(opt, 'fr'))
...
def select_option(option, selected_option, buttons):
...
if __name__ == "__main__":
    root = Tk()
    language_id = 8  # Replace with actual language ID (e.g., 1 for English)
