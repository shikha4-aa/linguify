...
def save_language_level(username, level, db, cur):
    try:
        ...
        if level == "Intermediate":
            start_quiz_page_eng_inter(username)
        elif level == "Advanced":
            start_quiz_page_eng_adv(username)
        elif level == "Beginner":
            start_session_page_eng_beg(username)
...
def start_quiz_page_eng_inter(username):
    os.system("python english_intermediate.py")

def start_quiz_page_eng_adv(username):
    os.system("python english_advanced.py")

def start_session_page_eng_beg(username):
    os.system("python session_eng_beg.py")

def show_language_level_selection(username):
    level_selection_window.title("Select Your English Proficiency Level")
