...
def display_question(window, question_data, question_index, total_questions, language_id, session_number, db, cur, img_bg, results):
    if question_index >= total_questions:
        show_results(results)  # Show the results after the last question
        return
...
# Function to show results in a pie chart
...
def start_session(username, language_id, session_number):
    session_window = Tk()
    session_window.geometry("1366x768")
    session_window.title(f"Session {session_number} - Language Learning")
    ...
    try:
        ...
        with mysql.connector.connect(
            ...
            if total_questions > 0:
                display_question(session_window, questions, 0, total_questions, language_id, session_number, db, cur, img_bg, results)
            else:
...
if __name__ == "__main__":
    ...
    language_id = 4  # Replace with actual language ID (e.g., 1 for English)
    session_number = 1  # Example session number
    start_session(username, language_id, session_number)
